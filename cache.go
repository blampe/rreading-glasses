...
// layeredcache implements a simple tiered cache. Hits at lower layers are
// automatically propagated up. Values are compressed with gzip at rest.
//
// cache.ChainCahce has inconsistent marshaling behavior, so we use our own
// wrapper. Actually that package doesn't really buy us anything...
type layeredcache struct {
	// wrapped []*cache.Cache[any]
...
func (c *layeredcache) GetWithTTL(ctx context.Context, key string) ([]byte, time.Duration, bool) {
	...
	for idx, cc := range c.wrapped {
		...
		if err != nil {
			// Propagate the value back if we eventually find it.
		...
		// TODO: Check if the client supports gzip content-encoding, in which
		// case we can just return compressed bytes as-is.
...
func newMemory() *cache.Cache[[]byte] {
	r, err := ristretto.NewCache(&ristretto.Config{
		NumCounters: 5e7,                                // number of keys to track frequency of (50M).
		MaxCost:     3 * (debug.SetMemoryLimit(-1) / 4), // 75% of our available memory.
