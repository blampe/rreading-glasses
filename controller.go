...
// Lower this quite a lot while we're beta testing.
var (
...
// controller facilitates operations on our cache. Importantly it schedules
// background work and handles cache invalidation.
//
...
// within the lifespan of the request.
//
// Another difference is that we only refresh an author's works if we notice a
// new work not already known to belong to them. This saves us from polling
// inactive or long-dead authors.
type controller struct {
	...
	group    singleflight.Group // Prevents redundant lookups.

	// TODO: Bloom filter for author/work association
	// Alternatively store works sorted on the author
}
...
type getter interface {
	...
	// TODO GetBook(ctx context.Context, bookID int64, existing bookRsc) (bookRsc, error) // Returns a serialized Work
}

func newUpstream(host string, cookie string, proxy string) (*http.Client, error) {
	upstream := &http.Client{
		Transport: throttledTransport{
			// TODO: Unauthenticated defaults to 1 request per minute -- probably too
			// generous.
...
func newController(cache *layeredcache, core getter, upstream *http.Client) (*controller, error) {
	c := &controller{
		cache:    cache,
		core:     core,
		upstream: upstream,
...
func (c *controller) getBook(ctx context.Context, bookID int64) ([]byte, error) {
	...
	if err != nil {
		log(ctx).Warn("problem getting book", "err", err)
		return nil, err
	}

	log(ctx).Debug("caching", "size", len(workBytes), "key", bookKey(bookID))
...
// getAuthor an AuthorResource with up to 20 works populated. We persist data
// locally for 2x the TTL we give the client because stale data can be used to
// speed up subsequent cache misses / refreshes.
//
...
// ensureAuthorWorks(id any, []works) // Would need to lock the cache key...
